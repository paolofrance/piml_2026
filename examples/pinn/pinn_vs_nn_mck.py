...
print(f"Saved: {os.path.join(RESULTS_DIR, 'pinn_vs_nn.png')}")
plt.show()
...
for _name, _pred in _ind_preds.items():
    ...
    _a1.plot(t_eval, _pred,  color=COLORS[_name],  lw=1.8,
             label=f"{_name}  (extrap RMSE={metrics[_name]['extrap']:.4f})")
    _a1.scatter(t_train, x_train, s=60, color=COLORS["True"], zorder=10,
                label=f"{N_TRAIN} training obs", edgecolors="white", linewidths=0.5)
    _a1.axvline(split, color="gray", ls=":", lw=1.4, label="train end")
    _a1.axvspan(split, t_eval[-1], alpha=0.07, color="gray")
    _a1.set_ylabel("x(t)", fontsize=11); _a1.legend(fontsize=9); _a1.grid(True, alpha=0.3)
    _a2.semilogy(t_eval, np.abs(_pred - x_true) + 1e-6, color=COLORS[_name], lw=1.6, label=_name)
    ...
    plt.tight_layout()
    _fp = os.path.join(RESULTS_DIR, f"pinn_vs_nn_{_name}.png")
