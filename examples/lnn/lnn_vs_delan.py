...
lnn_vs_delan.py
===============
LNN vs DeLaN vs VanillaNN on the spring pendulum (2-DOF conservative system).
...
Both LNN and DeLaN learn a Lagrangian and derive equations of motion via
Euler-Lagrange. The difference:
  - LNN  : unstructured scalar L(q,q̇) — no guarantee on mass matrix SPD
  - DeLaN: structured L = T−V, T uses Cholesky-parameterised SPD mass matrix
           and V ≥ 0 enforced via softplus — stronger physical guarantees

Key insight: both Lagrangian methods conserve energy by construction and
outperform VanillaNN in extrapolation. DeLaN's additional structure (SPD M,
non-negative V) makes it more robust, especially on multi-DOF systems where
an indefinite mass matrix can cause instability during rollout.
"""
...
for ax, ci, lbl in zip(axes[:2], [0, 1], ["x (m)", "y (m)"]):
    ...
    for name in _model_names:
        ...
        ax.plot(t_eval, xy_plot, color=COLORS[name], lw=1.8,
                label=f"{name}  (extrap={preds[name]['extrap']:.4f})")
...
print(f"Saved: {os.path.join(RESULTS_DIR, 'lnn_vs_delan.png')}")
plt.show()
...
for _name in _model_names:
    _p = preds[_name]
    _fi, _axes_i = plt.subplots(3, 1, figsize=(10, 10), sharex=True)
    _fi.suptitle(f"{_name} — Spring Pendulum 2-DOF\n"
                 f"{N_TRAIN} training pts from [0, {T_TRAIN[1]} s]  |  shaded = extrapolation",
                 fontsize=12, fontweight="bold")
    for _ax, _ci, _lbl in zip(_axes_i[:2], [0, 1], ["x (m)", "y (m)"]):
        _ax.plot(t_eval, xy_true[:, _ci], color=COLORS["True"], lw=1, ls="--",
                 label="Ground truth", zorder=5)
        _ax.plot(t_eval, np.clip(_p["xy"][:, _ci], -5, 5), color=COLORS[_name], lw=1.8,
                 label=f"{_name}  (extrap={_p['extrap']:.4f})")
        _ax.axvline(split, color="gray", ls=":", lw=1.4, label="train end")
        _ax.axvspan(split, t_eval[-1], alpha=0.07, color="gray")
        _ax.set_ylabel(_lbl, fontsize=11); _ax.legend(fontsize=9); _ax.grid(True, alpha=0.3)
    _err = np.sqrt(np.sum((np.clip(_p["xy"], -5, 5) - xy_true)**2, axis=1))
    _axes_i[2].semilogy(t_eval, _err + 1e-6, color=COLORS[_name], lw=1.6, label=_name)
    _axes_i[2].axvline(split, color="gray", ls=":", lw=1.4)
    _axes_i[2].axvspan(split, t_eval[-1], alpha=0.07, color="gray")
    _axes_i[2].set_ylabel("‖xy error‖  (log scale)", fontsize=11)
    _axes_i[2].set_xlabel("Time (s)", fontsize=11)
    _axes_i[2].legend(fontsize=9); _axes_i[2].grid(True, alpha=0.3)
    plt.tight_layout()
    _fp = os.path.join(RESULTS_DIR, f"lnn_vs_delan_{_name}.png")
...
fig_a.suptitle("LNN vs DeLaN vs VanillaNN — spring pendulum orbital motion", fontsize=11)
...
_ap = os.path.join(RESULTS_DIR, "lnn_vs_delan_anim.mp4")
try:
