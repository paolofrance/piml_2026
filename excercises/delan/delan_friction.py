...
# ---------------------------------------------------------------------------
# Animation — dissipative spring-pendulum orbital view
# ---------------------------------------------------------------------------
...
def _sp2d(ox, oy, bx, by, n=7, af=0.09):
    ...
    off = np.zeros(N); off[1:-1:2] = amp; off[2:-1:2] = -amp
    return xs+off*px, ys+off*py

fig_a, (ax_ph, ax_tr) = plt.subplots(1, 2, figsize=(13, 6))
fig_a.suptitle(
    f"DeLaN-F vs DeLaN vs VanillaNN — dissipative spring pendulum  (b_r={sp.b_r}, b_θ={sp.b_theta})",
    fontsize=11)
...
for nm in _pnames:
    sp_l, = ax_ph.plot([], [], '-', color=COLORS[nm], lw=1.4)
    ...
    _springs[nm]=sp_l; _bobs[nm]=bob; _trails[nm]=trail
...
_ap = os.path.join(RESULTS_DIR, "delan_friction_anim.mp4")
try:
